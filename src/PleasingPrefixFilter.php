...
{
  const FIT_FILL = [
      'fill-available' => [ '-webkit-fill-available', '-moz-available', 'fill-available' ],
      'fit-content'    => [ '-webkit-fit-content', '-moz-fit-content', 'fit-content' ],
      'max-content'    => [ '-webkit-max-content', '-moz-max-content', 'intrinsic', 'max-content' ],
      'min-content'    => [ '-webkit-min-content', '-moz-min-content', 'min-intrinsic', 'min-content' ],
  ];

  /** @var array CSS Properties where the value is prefixed. */
  const PREFIX_VALUES = [
      'display' => [
          'flex'        => [ '-webkit-flex', '-ms-flexbox', 'flex' ],
          'grid'        => [ '-ms-grid', 'grid' ],
          'inline-grid' => [ '-ms-inline-grid', 'inline-grid' ],
          'inline-flex' => [ '-webkit-inline-flex', '-ms-inline-flexbox', 'inline-flex' ]
      ],
  ...
  const PREFIX_PROPERTY = [
      'flex-direction'        => [ '-webkit-flex-direction', '-ms-flex-direction', 'flex-direction' ],
      'flex-grow'             => [ '-webkit-flex-grow', '-ms-flex-positive', 'flex-grow' ],
      'flex-shrink'           => [ '-webkit-flex-shrink', '-ms-flex-negative', 'flex-shrink' ],
      'flex-basis'            => [ '-webkit-flex-basis', '-ms-flex-preferred-size', 'flex-basis' ],
      'flex-wrap'             => [ '-webkit-flex-wrap', '-ms-flex-wrap', 'flex-wrap' ],
      'flex-flow'             => [ '-webkit-flex-flow', '-ms-flex-flow', 'flex-flow' ],
      'order'                 => [ '-webkit-order', '-ms-flex-order', 'order' ],
      'transition'            => [ '-webkit-transition', '-o-transition', 'transition' ],
      'box-sizing'            => [ '-webkit-box-sizing', 'box-sizing' ],
      'column-count'          => [ '-webkit-column-count', 'column-count' ],
      'column-gap'            => [ '-webkit-column-gap', 'column-gap' ],
      'column-width'          => [ '-webkit-column-width', 'column-width' ],
      'column-rule'           => [ '-webkit-column-rule', 'column-rule' ],
      ...
      'grid-row-start'        => [ '-ms-grid-row', 'grid-row-start' ],
      'grid-column-start'     => [ '-ms-grid-column', 'grid-column-start' ],
      'grid-column'           => [ '-ms-grid-column', 'grid-column' ],
      'grid-row'              => [ '-ms-grid-row', 'grid-row' ],
      'justify-self'          => [ '-ms-grid-row-align', 'justify-self' ],
      'justify-content'       => [ '-ms-flex-pack', '-webkit-justify-content', 'justify-content' ],
      'align-items'           => [ '-ms-flex-align', '-webkit-align-items', 'align-items' ],
      'align-self'            => [ '-ms-flex-item-align', '-webkit-align-self', 'align-self' ],
      'align-content'         => [ '-ms-flex-line-pack', '-webkit-align-content', 'align-content' ],
      'flex'                  => [ '-ms-flex', '-webkit-flex', 'flex' ]
  ];
  ...
  public function setPrefixProperties($prefixProperties)
  {
    ...
    {
      $defaults  = array_key_exists( $property, self::PREFIX_PROPERTY ) ? self::PREFIX_PROPERTY[$property] : [];
      $tPrefixes = [];
  ...
  public function setPrefixValues(array $prefixValue)
  {
    ...
    {
      $propDefaults = array_key_exists($prop, self::PREFIX_VALUES) ? self::PREFIX_VALUES[$prop] : [];
      foreach ( $values as $value => $prefixes )
      {
        $defaults  = array_key_exists( $value, $propDefaults ) ? $propDefaults[ $value ] : [];
        $tPrefixes = [];
        foreach ( $prefixes as $prefix )
        {
          if ( substr($prefix, -1, 1) === '*' )
